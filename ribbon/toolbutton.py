...
class ToolButton(QToolButton):
    ...
    @typing.overload
    def __init__(
        self, icon: QtGui.QIcon, text: str, style=ButtonStyle.Large, parent=None
    ):
    ...
    def __init__(self, *args, **kwargs):
        if (args and isinstance(args[0], QtGui.QIcon)) or (kwargs and "icon" in kwargs):
            self.buttonStyle = (
                args[2]
                if len(args) > 2
                else kwargs.get("buttonStyle", ButtonStyle.Large)
            )
        ...
        elif (args and isinstance(args[0], ButtonStyle)) or (
            kwargs and "buttonStyle" in kwargs
        ):
            self.buttonStyle = (
                args[0]
                if len(args) > 0
    ...
    def setButtonStyle(self, buttonStyle: ButtonStyle) -> None:
        ...
        if buttonStyle in [ButtonStyle.Large, ButtonStyle.Medium]:
            self.setIconSize(
                QSize(48, 48) if buttonStyle == ButtonStyle.Large else QSize(32, 32)
            )
    ...
    def mousePressEvent(self, e: QMouseEvent) -> None:
        if (
            ...
            and self.popupMode() == QStyleOptionToolButton.MenuButtonPopup
        ):
            if self.buttonStyle in [ButtonStyle.Large, ButtonStyle.Medium]:
    ...
    def sizeHint(self) -> QSize:
        ...
        if self.buttonStyle in [ButtonStyle.Large, ButtonStyle.Medium]:
            ...
            if size.width() > size.height() * 1.4:
                ...
                fm = self.fontMetrics()
                # 计算默认的文本区域
                textRange = self._calcTextRect(
                    QRect(0, 0, size.width() / 2, size.height())
                )
                ...
                if (opt.features & QStyleOptionToolButton.Menu) or (
    ...
    def _paintSmallButton(self) -> None:
        ...
        if (
            ...
            or option.features & QStyleOptionToolButton.HasMenu
        ):
        ...
        mFlags = bFlags
        if autoRaise:
            if not (bFlags & QStyle.State_MouseOver) or not (
            ...
            if option.activeSubControls & QStyle.SC_ToolButtonMenu:
                ...
                if autoRaise:
                    ...
                    self.style().drawPrimitive(
                        QStyle.PE_PanelButtonBevel, tool, p, self
                    )
                ...
                if tool.state & QStyle.State_MouseOver:
                    if self.mouseOnSubControl:  # 此时鼠标在indicate那
                        ...
                        if autoRaise:
                            self.style().drawPrimitive(
                                QStyle.PE_PanelButtonTool, tool, p, self
                            )
                        else:
                            self.style().drawPrimitive(
                                QStyle.PE_PanelButtonBevel, tool, p, self
                        ...
                        # 鼠标在图标区，把文字显示为正常
                        tool.state = QStyle.State_Raised  # 把图标区域显示为正常
                        tool.rect = option.rect.adjusted(
                            self.iconRect.width() + 1, 1, -1, -1
                        )
                        if autoRaise:
                            self.style().drawPrimitive(
                                QStyle.PE_PanelButtonTool, tool, p, self
    ...
    def _paintLargeButton(self) -> None:
        ...
        if (option.subControls & QStyle.SC_ToolButton) and (
            ...
            if option.activeSubControls & QStyle.SC_ToolButtonMenu:
                self.style().drawPrimitive(
                    QStyle.PE_PanelButtonTool, tool, painter, self
                )
                # 菜单激活
                self.style().drawPrimitive(
                    QStyle.PE_PanelButtonTool, tool, painter, self
                ...
                if autoRaise:
                    ...
                    self.style().drawPrimitive(
                        QStyle.PE_PanelButtonBevel, tool, painter, self
                    )
    ...
    def _drawIconAndLabel(
        self, painter: QPainter, option: QStyleOptionToolButton
    ) -> None:
        ...
        if self.buttonStyle in [ButtonStyle.Large, ButtonStyle.Medium]:
            ...
            if (
                not hasArrow
                and option.icon.isNull()
                and not len(option.text) == 0
                or option.toolButtonStyle == Qt.ToolButtonTextOnly
            ):
                # 没有箭头 且 没图标 且 有文字 or 只有文字模式
                alignment = (
                    Qt.AlignCenter | Qt.TextShowMnemonic | Qt.TextWordWrap
                )  # 纯文本下，居中对齐,换行
                if not self.style().styleHint(
                    QStyle.SH_UnderlineShortcut, option, self
                ):
                ...
                if not option.toolButtonStyle == Qt.ToolButtonIconOnly:
                    ...
                    if not hasArrow:
                        ...
                        self.style().drawItemPixmap(
                            painter, self.iconRect, Qt.AlignCenter, pm
                        )
                    else:
                        self._drawArrow(
                            self.style(), option, self.iconRect, painter, self
                        )
                    ...
                    if hasArrow:
                        self._drawArrow(
                            self.style(), option, option.rect, painter, self
                        )
                    else:
                        self.style().drawItemPixmap(
                            painter, self.iconRect, Qt.AlignCenter, pm
                ...
                if option.features & QStyleOptionToolButton.HasMenu:
                    ...
                    self.style().drawPrimitive(
                        QStyle.PE_IndicatorArrowDown, option, painter, self
                    )
        else:
            # 小图标
            # pmSize = opt.iconSize
            if not option.icon.isNull():
                ...
                if not option.toolButtonStyle == Qt.ToolButtonIconOnly:
                    ...
                    if option.toolButtonStyle == Qt.ToolButtonTextUnderIcon:
                        ...
                        self.style().drawItemPixmap(
                            painter,
                            QStyle.visualRect(option.direction, option.rect, pr),
                            Qt.AlignCenter,
                            pm,
                        )
                    ...
                    self.style().drawItemText(
                        painter,
                        QStyle.visualRect(option.direction, option.rect, tr),
                        alignment,
                        option.palette,
                        option.state & QStyle.State_Enabled,
                        option.text,
            ...
            if option.features & QStyleOptionToolButton.HasMenu:
                ...
                self.style().drawPrimitive(
                    QStyle.PE_IndicatorArrowDown, tool, painter, self
                )

    @staticmethod
    def _drawArrow(
        style: QStyle,
        toolButton: QStyleOptionToolButton,
        rect: QRect,
        painter: QPainter,
        widget: QWidget,
    ) -> None:
        ...
        if self.buttonStyle in [ButtonStyle.Large, ButtonStyle.Medium]:
            ...
            if not opt.toolButtonStyle == Qt.ToolButtonIconOnly:
                if self.buttonStyle == ButtonStyle.Large:
                    ...
                    iconRect.setHeight(
                        self._liteLargeButtonIconHeight(opt.rect.height())
                    )
        else:
            if opt.toolButtonStyle == Qt.ToolButtonIconOnly:
                ...
                if (
                    ...
                    or opt.features & QStyleOptionToolButton.HasMenu
                ):
                    # 如果有菜单
                ...
                iconRect = QRect(
                    0,
                    0,
                    max(opt.rect.height(), opt.iconSize.width()),
                    opt.rect.height(),
                )
        return iconRect
    ...
    def _calcTextRect(self, *args) -> QRect:
        if type(args[0]) == QStyleOptionToolButton:
            ...
            if opt.state & (QStyle.State_Sunken | QStyle.State_On):
                shiftX = self.style().pixelMetric(
                    QStyle.PM_ButtonShiftHorizontal, opt, self
                )
                shiftY = self.style().pixelMetric(
                    QStyle.PM_ButtonShiftVertical, opt, self
                )
            rect = self._calcTextRect(
                opt.rect,
                opt.features & QStyleOptionToolButton.HasMenu
                or opt.features & QStyleOptionToolButton.Menu,
            )
            ...
            if self.buttonStyle == ButtonStyle.Large:
                if hasMenu:
                    if self.isWordWrap:
                        ...
                        rect.adjust(
                            1,
                            buttonRect.height() * LARGE_BUTTON_ICON_RATIO,
                            -ARROW_WIDTH,
                            -1,
                        )
                    else:
                    ...
                    # 没有菜单不用预留箭头，1px的上下边界
                    rect.adjust(
                        1, buttonRect.height() * LARGE_BUTTON_ICON_RATIO, -1, -1
                    )
                ...
                if hasMenu:
                    rect.adjust(
                        1,
                        self._liteLargeButtonIconHeight(buttonRect.height()),
                        -ARROW_WIDTH,
                        -1,
                    )
                else:
                    rect.adjust(
                        1, self._liteLargeButtonIconHeight(buttonRect.height()), -1, -1
                    )
            else:
                if not self.toolButtonStyle() == Qt.ToolButtonIconOnly:
                    if hasMenu:
                        rect = buttonRect.adjusted(
                            self.iconRect.width(), 0, -ARROW_WIDTH, 0
                        )
                    else:
    ...
    def _createIconPixmap(self, opt: QStyleOptionToolButton) -> QPixmap:
        if not opt.icon.isNull():  # 有图标
            ...
            if not (opt.state & QStyle.State_Enabled):
                mode = QIcon.Disabled
            elif (opt.state & QStyle.State_MouseOver) and (
                opt.state & QStyle.State_AutoRaise
            ):
                mode = QIcon.Active
            else:
                mode = QIcon.Normal
            return opt.icon.pixmap(
